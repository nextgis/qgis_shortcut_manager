...
RE_ICON_REL = re.compile(
    r'rel*=\s*["\']?\s*(?:shortcut\s+)?icon[ "\'>]', re.I | re.S
)
...
if __name__ == "__main__":
    ...
    assert (
        find_favicon_from_dump('<link rel="icon" href="foo">', "http://ya.ru")
        == "http://ya.ru/foo"
    )
    assert (
        find_favicon_from_dump(
            '<link rel="icon" href="/foo">', "http://ya.ru/bar"
        )
        == "http://ya.ru/foo"
    )
